/**
 * TYPE_TAGS recognition + JSON record encoding for filesystem entities.
 *
 * Same canonical encoding as the S3 backend (`src/s3/fields.ts`): the
 * filesystem stores entity records as JSON files, so the JS↔JSON
 * adaptation logic is identical.
 *
 *   STRING    → JSON string
 *   NUMBER    → JSON number
