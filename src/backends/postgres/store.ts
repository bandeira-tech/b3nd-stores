 ...
 *
 * Pure mechanical storage with no protocol awareness. Uses an injected
 * SqlExecutor so the package does not depend on a specific Postgres
 * driver. All `fn=ls`/`fn=count` work is pushed down to SQL: the
 * shallow-direct-leaves contract (`uri LIKE prefix% AND uri NOT LIKE
 * prefix%/%`) is enforced in the WHERE clause.
 */
...
export class PostgresStore implements Store {
    ...
    for (const entry of entries) {
      try {
        ...
        await this.executor.query(
          `INSERT INTO ${this.tableName} (uri, data) VALUES ($1, $2::jsonb)
           ON CONFLICT (uri) DO UPDATE SET data = EXCLUDED.data, updated_at = CURRENT_TIMESTAMP`,
          [entry.uri, encoded],
        );
  ...
  private async _readOne(uri: string): Promise<unknown> {
    const res = await this.executor.query(
      `SELECT data FROM ${this.tableName} WHERE uri = $1`,
      [uri],
    );
    if (!res.rows || res.rows.length === 0) return undefined;
    const row = res.rows[0] as { data: unknown };
    return decodeBinaryFromJson(row.data);
  }
  ...
  private async _ls(parsed: ParsedUrl): Promise<Output[] | string[]> {
    ...
    const rows = (res.rows ?? []) as Array<{ uri: string; data?: unknown }>;

    if (format === "uris") return rows.map((r) => r.uri);
    return rows.map((r): Output => [r.uri, decodeBinaryFromJson(r.data)]);
  }
